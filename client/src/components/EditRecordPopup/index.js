...
const EditRecordPopup = ({record, domain, close, hostedZoneId, getHostedZoneRecords}) => {

    const [recordData, setRecordData] = useState({ name: record.Name.slice(0, record.Name.length - domain.length - 1), recordType: record.Type, value: record.ResourceRecords.map(record => record.Value).join(', '), ttl: record.TTL })
    ...
    const handleEditRecord = async (e) => {
        ...
        const updateRecord = {...recordData, name: domain === record.Name ? domain : recordData.name + '.' + domain.slice(0, domain.length - 1) + ".", value}
        console.log(updateRecord)
        // return
        const url = process.env.REACT_APP_API_URL + '/dns/update-record/' + hostedZoneId
        const options = {
            method: 'PUT',
            headers: {
    ...
    return (
        <form className="create-hosted-zone-popup" onSubmit={handleEditRecord}>
            <h1 className="create-hosted-zone-title">Edit Record</h1>
            <label htmlFor="name" className="homepage-label">RECORD NAME</label>
            <div className='homepage-input-con'>
                <input type="text" style={record.Name === domain ? {width: '100%'} : {}} id="name" name="name" className='edit-input' disabled={record.Name === domain} placeholder='subdomain' value={recordData.name} onChange={handleInputChange}/>
                {record.Name !== domain && <p className='homepage-input-con-text'>.{domain.slice(0, domain.length - 1)}</p>}
            </div>
            <label htmlFor="recordType" className="homepage-label">RECORD TYPE</label>
            <select id="recordType" name="recordType" disabled={record.Name === domain} className='homepage-input' value={recordData.recordType} onChange={handleInputChange}>
            ...
            <button type="submit" className='homepage-button'>Update</button>
